...
output:
  word_document:
    word_template: inst/doc/word-template-legal.docx
    
params:  # From TRACTS.XLSX
  LeaseID: 3
  LeaseNo: 101.02
editor: visual
---
...
```{r}
### Parameterized Reporting Using Quarton - Jadey Ryan
### https://www.youtube.com/watch?v=MKjz_xkMgxY&t=2093s

# knitr::knit_exit()
# `r format(as.Date(leases$LeaseDate, format = "%Y-%m-%d"), format = "%B, %d, %Y")
```
...
# tracts0 <- read_excel("F:/D_Documents/GitHub-DasRotRad/parameterized_OGLS/data/tracts.xlsx")

# tracts0 <- read_excel("E:/GTLS/Johnson/tallgrass-params-0/data/tracts.xlsx")
tracts0 <- read_excel(here::here("data/tracts.xlsx"))
# View(tracts0)
```
...
# leases2merge.xlxs is created in ogls_con.R
# ogls_con.R imports tables from Access 
library(tidyverse)
library(readxl)

# leases0 <- read_excel("F:/D_Documents/GitHub-DasRotRad/parameterized_OGLS/data/leases2merge.xlsx", 
# leases0 <- read_excel("E:/GTLS/Johnson/tallgrass-params-0/data/leases2merge.xlsx", 
...
library(readxl)
# leases0 <- read_excel("E:/GTLS/Johnson/tallgrass-params-0/data/leases2merge.xlsx")
leases0 <- read_excel(here::here("data/leases2merge.xlsx")) %>% 
  filter(LeaseID < 11)
# View(leases2merge)
View(leases0)
...
# library(formattable)
# leases$BonusPmt <- currency(leases$BonusPmt, digits = 2L)
# leases$SeisPmt <- currency(leases$SeisPmt, digits = 2L)
# leases$Totalbonus <- currency(leases$Totalbonus, digits = 2L)
...
# `r MASS::fractions(as.numeric(params$Royalty))` 
...
# * params work on leases tibble
# * If params from tracts tibble are included, R crashes.
params$LeaseID
# params$LessorsName
# params$LeaseNo
# params$LeaseTerm
...
# params$Rng
# params$Sec
# currency(params$BonusPmt, symbol="$", digits = 2L)
# currency(params$SeisPmt, symbol="$", digits = 2L)
# currency(params$Totalbonus, symbol="$", digits = 2L)
```

```{r}
# Text Alignment:  https://www.webucator.com/how-to/how-align-text-with-css.cfm   
# 
```

```{r eval=FALSE}
#  `r format(params$LeaseDate, format = "%d %B %Y")`
```
...
![](images/signatureRCC.PNG){fig-align="left" width="191" height="82"}
